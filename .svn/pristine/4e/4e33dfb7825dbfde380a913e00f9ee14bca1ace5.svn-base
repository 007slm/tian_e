...
import com.orange.browser.TabControl.OnThumbnailUpdatedListener;
...
public class Tab implements PictureListener {
    ...
    // FIXME: We do not use mTouchIconLoader to download touch icon.
    // We download touch icon when save bookmark.
...
//                mContext.sendBroadcast(logIntent);
    ...
    private static int PAGE_LOAD = 0;
    private static int PAGE_REDIRECT = 1;
    private static int PAGE_FINISH = 2;
    
    private final WebViewClient mWebViewClient = new WebViewClient() {
        private Message mDontResend;
        private Message mResend;
        private boolean mStartState = false;
        private boolean mFinishState = true;
        private boolean mRedirectMode = false;
        private int mInitState = PAGE_FINISH;
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            if (false){ //It will introduce many problems, so we will implement homepage landscape.
            //Because homepage has bugs between portrait and landscape, we forbid landscape now.
            if (BrowserSettings.getInstance().getHomePage().equals(url)){
                mWebViewController.getTabControl().getBrowserActivity().
            ...
            // For ICS, we do not use hardware acceleration for homepage first time.
//            if (BrowserGlobals.isPlatformHoneycombAndAbove()) {
//                if (BrowserGlobals.isFirstTime()) {
//                    view.setLayerType(View.LAYER_TYPE_SOFTWARE, null);
//                    BrowserGlobals.removeFirstTime();
//                } else if (view.getLayerType() == View.LAYER_TYPE_SOFTWARE)
//                    view.setLayerType(View.LAYER_TYPE_NONE, null);
//            }
            //
            if (mInitState == PAGE_FINISH) {
                mInitState = PAGE_LOAD;
                mRedirectMode = false;
            }
            mStartState = true;
            mFinishState = false;
...
//                mContext.sendBroadcast(logIntent);
            ...
            if (mInForeground) {
                boolean filterFlag = mWebViewController.shouldOverrideUrlLoading(Tab.this,
                        view, url);
                if (BrowserGlobals.isChinaVersion(mContext)) {
                    ...
                    if (filterFlag) {
                        ...
                        if (!mStartState && !mFinishState) {
                            view.loadUrl(UrlUtils.encodeUrl(url));
                            return true;
                        } else if (mStartState && mRedirectMode && !mFinishState) {
                            view.loadUrl(UrlUtils.encodeUrl(url));
                            return true;
                        } else {
                            return false;
        ...
        public void onReceivedError(WebView view, int errorCode,
            ...
            if (!isPrivateBrowsingEnabled()) {
                ...
                if (isHomePage()) {
                    view.loadUrl(BrowserSettings.getErrorPage());
                }
            }
        }
...
//                                handleProceededAfterSslError(error);
         ...
         */
        // The following interface does not exist in Android 2.3.
        // So, ignore it.
...
//    }
    ...
    private final WebChromeClient mWebChromeClient = new WebChromeClient() {
        ...
        public boolean onCreateWindow(WebView view, final boolean dialog,
            ...
            if (mWebViewController.isCollectorMode()) {
                return false;
            }
            if (!mInForeground) {
                return false;
...
//        }
        ...
        public boolean onConsoleMessage(ConsoleMessage consoleMessage) {
            ...
            //Do not turn off the debug info below.
            //It is used by html and should be controlled by html.
        ...
        public void onSelectionStart(WebView view) {
            // Do nothing to enable text selection.
        }
...
//            mMainView.setWebBackForwardListClient(mWebBackForwardListClient);
            ...
            if (mSavedState != null) {
                ...
                mSavedState = null;
                //NOTE: Android WebView restoreState will not restore Javascript
...
//            mMainView.setEmbeddedTitleBar(null);
    ...
    boolean createSubWindow() {
        if (mSubView == null) {
            ...
            //add by gaoge 2012-02-06 for android2.3 implemention
...
//                mSubView.onResume();
    ...
    private void setupHwAcceleration(View web) {
        ...
        if (!isHomePage() && settings.isHardwareAccelerated()) {
...
//                mSubView.onPause();
    ...
    void putInForeground() {
        ...
        mInForeground = true;
        resume();
        BrowserActivity activity = (BrowserActivity) mWebViewController.getActivity();
    ...
    String getOriginalUrl() {
        if (mCurrentState.mOriginalUrl == null) {
            LogHelper.d(LogHelper.TAG_STARBTN, "getOriginalUrl,originalUrl is null");
            return getUrl();
        }
    ...
    public Bundle saveState() {
        ...
        mSavedState.putBoolean(INCOGNITO, false);
        if (mAppId != null) {
    ...
    private void restoreState(Bundle b) {
        ...
        mId = b.getLong(ID);
        LogHelper.d(LOGTAG, "restoreState,mId is: " + mId);
...
//        c.setBitmap(null);
        ...
        synchronized (Tab.this) {
            ...
            try {
                ...
                Log.e(LOGTAG, "Load capture has mismatched sizes; buffer: "
                        ...
                        + "capture: " + mCapture.getRowBytes() * mCapture.getHeight());
                throw rex;
            }
        }
    }
    ...
    private byte[] getCaptureBlob() {
        synchronized (Tab.this) {
            ...
            if (buffer == null || buffer.limit() < mCapture.getRowBytes() * mCapture.getHeight()) {
                buffer = ByteBuffer.allocate(mCapture.getRowBytes() * mCapture.getHeight());
                sBuffer.set(buffer);
            }
    ...
    private Runnable mSaveThumbnail = new Runnable() {
        ...
        public void run() {
            ...
            //judge the thumbnail is exist.if true update thumbnail,else insert.
            Cursor cursor=cr.query(Thumbnails.CONTENT_URI, null, "_id="+mId, null, null);
            if(cursor!=null&&cursor.getCount()!=0){
                cursor.close();
                ContentValues values = new ContentValues();
                values.put(Thumbnails.THUMBNAIL, blob);
                cr.update(Thumbnails.CONTENT_URI, values, "_id="+mId, null);
            }else{
                ContentValues values = new ContentValues();
...
//    }
     ...
     */
    WebView showDialog(WebDialog dialog, int bottomHeight) {

        RelativeLayout container;
        WebView view;
        if (mSubView != null) {
            view = mSubView;
            container = (RelativeLayout) mSubViewContainer.findViewById(
        ...
        RelativeLayout.LayoutParams params2 = new
                RelativeLayout.LayoutParams(
                        ViewGroup.LayoutParams.MATCH_PARENT,
                        view_wrapper.getHeight() + bottomHeight);
    ...
    public boolean isHomePage() {
        return BrowserSettings.getInstance().getHomePage().equals(getUrl());
    }
