...
import com.orange.browser.provider.BrowserProvider.OmniboxSuggestions;
...
//                new SlowFilterTask().execute(constraint);
        ...
        protected void publishResults(CharSequence constraint, FilterResults fresults) {
            if (fresults != null && fresults.values instanceof SuggestionResults) {
    ...
    class SuggestCursor extends CursorSource {
        ...
        public SuggestItem getItem() {
            if (mCursor != null) {
                int index = mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_1);
                String title,text2,url,uri;
                ...
                index = mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_2);
                if(index != -1) {
                    text2 = mCursor.getString(index);
                } else {
                    text2 = "";
                }
                
                index = mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_TEXT_2_URL);
                if(index != -1) {
                ...
                index = mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_INTENT_DATA);
                if(index != -1) {
                    uri = mCursor.getString(index);
                } else {
                    uri = "";
                }
                ...
                index = mCursor.getColumnIndex(SearchManager.SUGGEST_COLUMN_INTENT_EXTRA_DATA);
                if(index != -1) {
                    item.extra = mCursor.getString(index);
                } else {
                    item.extra = "";
                }
