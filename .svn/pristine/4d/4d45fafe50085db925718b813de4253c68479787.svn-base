...
public class UrlHandler {
    ...
    private final static String SCHEME_APP = "app:";
    ...
    boolean shouldOverrideUrlLoading(Tab tab, WebView view, String url) {
        ...
        */
        ...
        if(mController.isCollectorMode()) {
            //TODO:HAN Liang add readerlist


            final Message msg = mController.getEffieientReadMode().getHandler().obtainMessage(
                    EffieientReadMode.GET_NODE_HREF);
            Bundle bundle = new Bundle();
            bundle.putString("originurl", url);
            msg.setData(bundle);
        ...
        if (url.startsWith(SCHEME_APP)) {
            ...
            try {
                mController.getContext().startActivity(i);
            } catch (Exception e) {
                Toast.makeText(mActivity, mActivity.getString(R.string.cannot_open_app),
        ...
        // To be more robust.
        // Do something if there is no market appliction.
        if (url.startsWith("market:")) {
            String marketUrl = url.replace("market://", "http://market.android.com/");
            mController.loadUrl(tab, marketUrl);
            return true;
        }
